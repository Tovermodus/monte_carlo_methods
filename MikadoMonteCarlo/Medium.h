...
class TrialMedium {
	...
	void random_movement(const double & time_step, const std::mt19937 & rng);
};
class Medium {
    private:
	std::vector<Rod> rods;
	Rod create_random_rod(const std::mt19937 & rng) const;
	...
	explicit Medium(const MediumParameters &params, const std::mt19937 & rng);
	void initialize_rods(const std::mt19937 & rng);
	double calculate_energy() const;
